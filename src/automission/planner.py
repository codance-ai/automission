...
def render_verify_sh(draft: PlanDraft) -> str:
    """Render verify.sh from PlanDraft."""
    return (
        "#!/usr/bin/env bash\n"
        "set -euo pipefail\n"
        'cd "$(dirname "$0")"\n'
...
- verify_command must be a single, complete, runnable shell command (e.g., "pytest tests/ -v")
...
class Planner:
    ...
    def _parse_response(self, raw: dict) -> PlanDraft:
        ...
        return PlanDraft(
            ...
            groups=groups,
            verify_command=raw.get("verify_command", "echo 'no tests configured'"),
